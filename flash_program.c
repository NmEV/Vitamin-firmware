...
// WARNING: reference-only demo (it has its own main() and is NOT part of the
// usbnet build). It erases and programmes the SAME 256 KB flash region that
// usbnet's encrypted storage (storage.c) uses: flashing it onto a board that
// runs usbnet will wipe the stored web-service data. Read it to understand
// the flash_safe_execute pattern; do not flash it on a live device.
